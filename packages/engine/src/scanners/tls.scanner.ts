...
export class TlsScanner implements BaseScanner {
  public readonly id = 'network:smtp:tls';
  public readonly description = 'Validates SMTP STARTTLS support, TLS version, cipher, and certificate expiry on the primary MX.';

  async execute(domain: string, presetMx?: string): Promise<ScannerResult> {
    ...
    return new Promise((resolve) => {
      let resolved = false;
      const finish = (result: Partial<ScannerResult>) => {
        if (resolved) return;
        resolved = true;
