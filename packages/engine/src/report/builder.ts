...
import { ReportModel, PresentationSection } from './types';
...
export class ReportBuilder {
  ...
  private static mapResultToSection(
    ...
    fallbackDesc: string,
    successExtractor: (res: ScannerResult) => string
  ): PresentationSection {
    if (!result) {
      return {
  ...
  private static buildAuthSections(report: EngineReport): PresentationSection[] {
    return [
      ...
      this.mapResultToSection('DomainKeys Identified Mail (DKIM)', report.scannerResults['auth:dkim'], 'Missing DKIM configuration.', (r: any) => `Valid DKIM. Selector: ${r.rawData?.selector || 'Unknown'} (${r.rawData?.type || 'rsa'})`),
      this.mapResultToSection('DMARC Enforcement', report.scannerResults['auth:dmarc'], 'Missing DMARC policy.', (r: any) => `Valid DMARC Configuration: ${r.rawData}`)
    ];
  }
  ...
  private static buildInfraSections(report: EngineReport): PresentationSection[] {
    return [
      ...
      this.mapResultToSection('Mail Exchange (MX)', report.scannerResults['network:mx'], 'Cannot receive email/bounces.', (r: any) => `${r.rawData?.length || 0} active MX routes defined.`),
      this.mapResultToSection('SMTP & TLS Security', report.scannerResults['network:smtp:tls'], 'MX Server failed STARTTLS negotiation.', (r: any) => `Secured via ${r.rawData?.protocol} (${r.rawData?.cipher}). Cert valid for ${r.rawData?.daysRemaining} days.`),
