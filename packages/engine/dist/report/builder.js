export class ReportBuilder {
    ...
    static buildAuthSections(report) {
        return [
            ...
            this.mapResultToSection('DomainKeys Identified Mail (DKIM)', report.scannerResults['auth:dkim'], 'Missing DKIM configuration.', (r) => `Valid DKIM. Selector: ${r.rawData?.selector || 'Unknown'} (${r.rawData?.type || 'rsa'})`),
            this.mapResultToSection('DMARC Enforcement', report.scannerResults['auth:dmarc'], 'Missing DMARC policy.', (r) => `Valid DMARC Configuration: ${r.rawData}`)
        ];
    }
    static buildInfraSections(report) {
        return [
            ...
            this.mapResultToSection('Mail Exchange (MX)', report.scannerResults['network:mx'], 'Cannot receive email/bounces.', (r) => `${r.rawData?.length || 0} active MX routes defined.`),
            this.mapResultToSection('SMTP & TLS Security', report.scannerResults['network:smtp:tls'], 'MX Server failed STARTTLS negotiation.', (r) => `Secured via ${r.rawData?.protocol} (${r.rawData?.cipher}). Cert valid for ${r.rawData?.daysRemaining} days.`),
