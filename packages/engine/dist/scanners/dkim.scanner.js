...
export class DkimScanner {
    id = 'auth:dkim';
    description = 'Validates DomainKeys Identified Mail (DKIM) records, key sizes, and syntax (via DoH).';
    static COMMON_SELECTORS = ['google', 'selector1', 's1', 'default', 'm1', 'mail', 'k1'];
    async execute(domain, knownSelector) {
        ...
        for (const selector of selectorsToCheck) {
            ...
            try {
                ...
                const combined = records.map((r) => r.data ? r.data.replace(/"/g, '') : '');
                const dkim = combined.find((r) => r.startsWith('v=DKIM1') || r.includes('p='));
                if (dkim) {
                    foundRecord = dkim;
