...
export class DmarcScanner {
    id = 'auth:dmarc';
    description = 'Validates DMARC record positioning and policy strictness (via DoH).';
    async execute(domain) {
        const targetDomain = `_dmarc.${domain}`;
        try {
            const records = await DoHClient.resolve(targetDomain, 'TXT');
            const dmarcRecords = records.map((r) => r.data ? r.data.replace(/"/g, '') : '').filter((record) => record.toLowerCase().startsWith('v=dmarc1'));
